# DelegateReaderStream - класс
Поток, чтение из которого делегируется заданной функции. Не поддерживает
запись в поток и свободное перемещение по нему при чтении [Seek(Int64,
...
C# __Копировать
     public sealed class DelegateReaderStream : Stream
VB __Копировать
     Public NotInheritable Class DelegateReaderStream
    	Inherits Stream
C++ __Копировать
     public ref class DelegateReaderStream sealed : public Stream
F# __Копировать
     [<SealedAttribute>]
    type DelegateReaderStream = 
        class
            inherit Stream
        end
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __[MarshalByRefObject](https://learn.microsoft.com/dotnet/api/system.marshalbyrefobject) __[Stream](https://learn.microsoft.com/dotnet/api/system.io.stream) __ DelegateReaderStream
##  __Конструкторы
[DelegateReaderStream](M_Tessa_Platform_IO_DelegateReaderStream__ctor.htm)|
Создаёт экземпляр класса с указанием функции, загружающей данные для потока.  
---|---  
## __Свойства
[BytesRemains](P_Tessa_Platform_IO_DelegateReaderStream_BytesRemains.htm)|
Количество байт, которое можно прочитать из потока с текущей позиции.  
---|---  
[CanRead](P_Tessa_Platform_IO_DelegateReaderStream_CanRead.htm)| Признак того,
что поток поддерживает чтение данных.  
(Переопределяет
[Stream.CanRead](https://learn.microsoft.com/dotnet/api/system.io.stream.canread#system-
io-stream-canread))  
[CanSeek](P_Tessa_Platform_IO_DelegateReaderStream_CanSeek.htm)| Признак того,
что текущую позицию в потоке можно изменять.  
(Переопределяет
...
[CanWrite](P_Tessa_Platform_IO_DelegateReaderStream_CanWrite.htm)|  Признак
того, что в поток можно записывать данные методом [System.IO.Stream.Write].
Свойство всегда возвращает false.  
(Переопределяет
[Stream.CanWrite](https://learn.microsoft.com/dotnet/api/system.io.stream.canwrite#system-
io-stream-canwrite))  
[Length](P_Tessa_Platform_IO_DelegateReaderStream_Length.htm)| Общее
количество доступных в потоке байт.  
(Переопределяет
[Stream.Length](https://learn.microsoft.com/dotnet/api/system.io.stream.length#system-
io-stream-length))  
[Position](P_Tessa_Platform_IO_DelegateReaderStream_Position.htm)| Текущая
позиция в потоке. Поддерживается только чтение свойства.  
(Переопределяет
...
(Унаследован от
[Stream](https://learn.microsoft.com/dotnet/api/system.io.stream))  
[DisposeAsync](M_Tessa_Platform_IO_DelegateReaderStream_DisposeAsync.htm)|
...
(Унаследован от
[Object](https://learn.microsoft.com/dotnet/api/system.object))  
[Flush](M_Tessa_Platform_IO_DelegateReaderStream_Flush.htm)|  Записывает все
данные из буфера потока на устройство и очищает буфер. Метод не
поддерживается.  
(Переопределяет
...
[Stream](https://learn.microsoft.com/dotnet/api/system.io.stream))  
[Read(Byte[], Int32,
Int32)](M_Tessa_Platform_IO_DelegateReaderStream_Read.htm)| Считывает данные
из потока и записывает их в заданный массив байт.  
...
CancellationToken)](M_Tessa_Platform_IO_DelegateReaderStream_ReadAsync.htm)|
...
(Унаследован от
[Stream](https://learn.microsoft.com/dotnet/api/system.io.stream))  
[Seek](M_Tessa_Platform_IO_DelegateReaderStream_Seek.htm)| Изменяет текущую
позицию в потоке. Метод не поддерживается.  
...
[SetLength](M_Tessa_Platform_IO_DelegateReaderStream_SetLength.htm)|
...
[Stream](https://learn.microsoft.com/dotnet/api/system.io.stream))  
[Write(Byte[], Int32,
Int32)](M_Tessa_Platform_IO_DelegateReaderStream_Write.htm)| Записывает данные
в поток из массива байт. Метод не поддерживается.  
...
[Stream](https://learn.microsoft.com/dotnet/api/system.io.stream))  
##  __Поля
[DefaultBufferSize](F_Tessa_Platform_IO_DelegateReaderStream_DefaultBufferSize.htm)|
Размер буфера по умолчанию - 1 МБ.  
---|---  
## __Методы расширения
