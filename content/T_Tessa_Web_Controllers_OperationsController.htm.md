...
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase) __[Controller](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controller) __ OperationsController
##  __Конструкторы
[OperationsController](M_Tessa_Web_Controllers_OperationsController__ctor.htm)|
Инициализирует новый экземпляр класса OperationsController  
---|---  
##  __Свойства
...
(Унаследован от
[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase))  
[Delete](M_Tessa_Web_Controllers_OperationsController_Delete.htm)|  Удаляет
операцию с заданным идентификатором. Операция перестаёт считаться активной.  
[DeleteOlderThan](M_Tessa_Web_Controllers_OperationsController_DeleteOlderThan.htm)|
Удаляет все активные операции, дата создания которых старше указанной даты.
Возвращает количество удалённых операций. Метод доступен только
администраторам.  
...
(Унаследован от
[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase))  
[GetAll](M_Tessa_Web_Controllers_OperationsController_GetAll.htm)|  Возвращает
список всех активных операций указанного типа typeID или вообще всех операций,
если тип не указан. Возвращаемый список может быть равен null, если операций
нет.  
[GetID](M_Tessa_Web_Controllers_OperationsController_GetID.htm)|  Возвращает
идентификатор операции по хеш-значению запроса с параметрами операции или null
(204 No Content), если соответствующей операции не было найдено.  
[GetOperation](M_Tessa_Web_Controllers_OperationsController_GetOperation.htm)|
Возвращает информацию по операции или null (204 No Content), если операция не
найдена.  
[GetState](M_Tessa_Web_Controllers_OperationsController_GetState.htm)|
Возвращает состояние и прогресс операции с заданным идентификатором или null
(204 No Content), если операция не считается активной, например, была удалена
при завершении.  
[IsAlive](M_Tessa_Web_Controllers_OperationsController_IsAlive.htm)|
Проверяет, что заданная операция ещё существует как активная. При этом
операция может быть не запущена, может быть запущена и может быть завершена
(если для операции установлено, что она должна завершаться без удаления
методом [PostComplete(Guid, OperationResponse, Boolean,
CancellationToken)](M_Tessa_Web_Controllers_OperationsController_PostComplete.htm)).  
...
(Унаследован от
[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase))  
[PostComplete](M_Tessa_Web_Controllers_OperationsController_PostComplete.htm)|
Завершает операцию с указанным идентификатором и объектом, описывающим
результат операции. Операция при этом не удаляется, чтобы процесс, запустивший
операцию, мог запросить результаты операции.  
[PostCreate](M_Tessa_Web_Controllers_OperationsController_PostCreate.htm)|
Создаёт операцию с заданными параметрами. Операция может быть создана сразу в
состоянии "Выполняется", если это указано в флаге
[CreateInProgress](T_Tessa_Platform_Operations_OperationCreationFlags.htm).
Возвращает идентификатор созданной операции.  
[PostReportProgress](M_Tessa_Web_Controllers_OperationsController_PostReportProgress.htm)|
Устанавливает прогресс операции в процентах. Операция должна быть запущена с
флагом
[ReportsProgress](T_Tessa_Platform_Operations_OperationCreationFlags.htm).
Возвращает признак того, что подходящая операция найдена, и её прогресс
установлен.  
[PostStart](M_Tessa_Web_Controllers_OperationsController_PostStart.htm)|
Запускает ранее созданную операцию, если она ещё не была запущена.  
[PostStartFirst](M_Tessa_Web_Controllers_OperationsController_PostStartFirst.htm)|
Запускает первую доступную операции для заданного типа, и возвращает
идентификатор этой операции или null (204 No Content), если доступных для
запуска операций нет. Метод потокобезопасен. Гарантируется, что при
параллельном обращении из разных потоков или разных веб-серверов к серверу
СУБД, только один поток или сервер получит запущенную операцию.  
