# KrPermissionExtensions.GetOrCreateServerToken - метод
##  __Definition
 **Пространство имён:**
...
C# __Копировать
     public static KrToken GetOrCreateServerToken(
    	this IDictionary<string, Object> info
    )
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function GetOrCreateServerToken ( 
    	info As IDictionary(Of String, Object)
    ) As KrToken
C++ __Копировать
     public:
    [ExtensionAttribute]
    static KrToken^ GetOrCreateServerToken(
    	IDictionary<String^, Object^>^ info
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member GetOrCreateServerToken : 
