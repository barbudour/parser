...
VB __Копировать
    <SerializableAttribute>
    Public NotInheritable Class WorkflowProcessStorage
    	Inherits WorkflowTemplateStorageWithDescriptionBase
C++ __Копировать
    [SerializableAttribute]
    public ref class WorkflowProcessStorage sealed : public WorkflowTemplateStorageWithDescriptionBase
F# __Копировать
     [<SealedAttribute>]
    [<SerializableAttribute>]
    type WorkflowProcessStorage = 
        class
            inherit WorkflowTemplateStorageWithDescriptionBase
        end
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __[StorageObject](T_Tessa_Platform_Storage_StorageObject.htm) __[WorkflowStorageBase](T_Tessa_Workflow_Storage_WorkflowStorageBase.htm) __[WorkflowTemplateStorageBase](T_Tessa_Workflow_Storage_WorkflowTemplateStorageBase.htm) __[WorkflowTemplateStorageWithDescriptionBase](T_Tessa_Workflow_Storage_WorkflowTemplateStorageWithDescriptionBase.htm) __ WorkflowProcessStorage
##  __Конструкторы
[WorkflowProcessStorage()](M_Tessa_Workflow_Storage_WorkflowProcessStorage__ctor.htm)|
Инициализирует новый экземпляр класса WorkflowProcessStorage  
---|---  
[WorkflowProcessStorage(IStorageObjectProvider)](M_Tessa_Workflow_Storage_WorkflowProcessStorage__ctor_2.htm)|
Инициализирует новый экземпляр класса WorkflowProcessStorage  
[WorkflowProcessStorage(Dictionary<String, Object>,
Nullable<Guid>)](M_Tessa_Workflow_Storage_WorkflowProcessStorage__ctor_1.htm)|
Инициализирует новый экземпляр класса WorkflowProcessStorage  
##  __Свойства
[Annotations](P_Tessa_Workflow_Storage_WorkflowProcessStorage_Annotations.htm)|
...
[GlobalScript](P_Tessa_Workflow_Storage_WorkflowProcessStorage_GlobalScript.htm)|
Глобальные скрипты процесса.  
[GraphOnly](P_Tessa_Workflow_Storage_WorkflowProcessStorage_GraphOnly.htm)|
Флаг определяет, что заданный шаблон бизнес-процесса содержит только
информацию о расположении узлов.  
...
(Унаследован от
[WorkflowStorageBase](T_Tessa_Workflow_Storage_WorkflowStorageBase.htm))  
[Links](P_Tessa_Workflow_Storage_WorkflowProcessStorage_Links.htm)|  Список
связей процесса.  
[LinksByID](P_Tessa_Workflow_Storage_WorkflowProcessStorage_LinksByID.htm)|
Связи шаблона бизнес-процесса, доступные по идентификатору связи. Свойство
доступно только после вызова метода
[Initialize()](M_Tessa_Workflow_Storage_WorkflowProcessStorage_Initialize.htm).  
[LogLevel](P_Tessa_Workflow_Storage_WorkflowProcessStorage_LogLevel.htm)|
Уровень логирования.  
[Name](P_Tessa_Workflow_Storage_WorkflowTemplateStorageBase_Name.htm)|  Имя
объекта.  
(Унаследован от
[WorkflowTemplateStorageBase](T_Tessa_Workflow_Storage_WorkflowTemplateStorageBase.htm))  
[NodeIDsByDefaultSignalType](P_Tessa_Workflow_Storage_WorkflowProcessStorage_NodeIDsByDefaultSignalType.htm)|
Доступные по типу отправляемого сигнала списки идентификаторов узлов процесса,
имеющие подписки по умолчанию на указанный тип сигнала. Свойство доступно
только после вызова метода
[Initialize()](M_Tessa_Workflow_Storage_WorkflowProcessStorage_Initialize.htm).  
[NodeIDsByStartSignalType](P_Tessa_Workflow_Storage_WorkflowProcessStorage_NodeIDsByStartSignalType.htm)|
Доступные по типу отправляемого сигнала списки идентификаторов узлов процесса,
имеющие подписки на запуск процесса с указанным типом сигнала. Свойство
доступно только после вызова метода
[Initialize()](M_Tessa_Workflow_Storage_WorkflowProcessStorage_Initialize.htm).  
[Nodes](P_Tessa_Workflow_Storage_WorkflowProcessStorage_Nodes.htm)|  Список
узлов процесса.  
[NodesByID](P_Tessa_Workflow_Storage_WorkflowProcessStorage_NodesByID.htm)|
Узлы шаблона бизнес-процесса, доступные по идентификатору узла. Свойство
доступно только после вызова метода
[Initialize()](M_Tessa_Workflow_Storage_WorkflowProcessStorage_Initialize.htm).  
...
(Унаследован от
[WorkflowTemplateStorageBase](T_Tessa_Workflow_Storage_WorkflowTemplateStorageBase.htm))  
[ParentTypeID](P_Tessa_Workflow_Storage_WorkflowProcessStorage_ParentTypeID.htm)|
ID базового класса процесса.  
[ParentTypeName](P_Tessa_Workflow_Storage_WorkflowProcessStorage_ParentTypeName.htm)|
Имя базового класса процесса.  
[Subscriptions](P_Tessa_Workflow_Storage_WorkflowProcessStorage_Subscriptions.htm)|
Список статических подписок процесса.  
[TemplateCardID](P_Tessa_Workflow_Storage_WorkflowProcessStorage_TemplateCardID.htm)|
ID карточки шаблона.  
## __Методы
...
[WorkflowStorageBase](T_Tessa_Workflow_Storage_WorkflowStorageBase.htm))  
---|---  
[CheckName](M_Tessa_Workflow_Storage_WorkflowProcessStorage_CheckName.htm)|
Метод проверяет, допустимо ли новое указанное имя для изменяемого объекта.  
(Переопределяет [WorkflowTemplateStorageBase.CheckName(String,
WorkflowTemplateStorageBase)](M_Tessa_Workflow_Storage_WorkflowTemplateStorageBase_CheckName.htm))  
...
(Унаследован от
[WorkflowStorageBase](T_Tessa_Workflow_Storage_WorkflowStorageBase.htm))  
[ClearChangesGlobal](M_Tessa_Workflow_Storage_WorkflowProcessStorage_ClearChangesGlobal.htm)|
Метод для очистки информации об изменении объекта.  
[Clone](M_Tessa_Workflow_Storage_WorkflowProcessStorage_Clone.htm)|  Метод для
создания клона объекта  
(Переопределяет
[WorkflowStorageBase.Clone()](M_Tessa_Workflow_Storage_WorkflowStorageBase_Clone.htm))  
...
(Унаследован от
[WorkflowTemplateStorageBase](T_Tessa_Workflow_Storage_WorkflowTemplateStorageBase.htm))  
[EnsureCacheResolved](M_Tessa_Workflow_Storage_WorkflowProcessStorage_EnsureCacheResolved.htm)|
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[GetDataForSave](M_Tessa_Workflow_Storage_WorkflowProcessStorage_GetDataForSave.htm)|
Метод для получения данных шаблона бизнес-процесса для сохранения.  
...
(Унаследован от
[WorkflowStorageBase](T_Tessa_Workflow_Storage_WorkflowStorageBase.htm))  
[HasChangesGlobal](M_Tessa_Workflow_Storage_WorkflowProcessStorage_HasChangesGlobal.htm)|
Метод для проверки наличия изменений в объекте.  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[Initialize](M_Tessa_Workflow_Storage_WorkflowProcessStorage_Initialize.htm)|
Метод для инициализации свойств объекта для быстрого доступа. Выполняется
только один раз.  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[IsValidStorage](M_Tessa_Workflow_Storage_WorkflowProcessStorage_IsValidStorage.htm)|
Метод для проверки, что переданное хранилище данных является допустимым.  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[RemoveData](M_Tessa_Workflow_Storage_WorkflowProcessStorage_RemoveData.htm)|
Метод для удаления данных из шаблона.  
(Переопределяет
[WorkflowTemplateStorageBase.RemoveData()](M_Tessa_Workflow_Storage_WorkflowTemplateStorageBase_RemoveData.htm))  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[UpdateData](M_Tessa_Workflow_Storage_WorkflowProcessStorage_UpdateData.htm)|  
(Переопределяет
[WorkflowTemplateStorageWithDescriptionBase.UpdateData(WorkflowStorageBase)](M_Tessa_Workflow_Storage_WorkflowTemplateStorageWithDescriptionBase_UpdateData.htm))  
##  __События
