...
совместно с пакетом карточек. Метаинформация запрашивается у сервиса при
первом обращении.
## __Definition
...
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __ CardCachedMetadata
Implements
    [ICardCachedMetadata](T_Tessa_Cards_ICardCachedMetadata.htm), [ICardMetadata](T_Tessa_Cards_ICardMetadata.htm), [ISealable](T_Tessa_Platform_ISealable.htm)
##  __Заметки
Если объект защищён от изменения методом
[Seal()](M_Tessa_Cards_Metadata_CardCachedMetadata_Seal.htm) сразу после
создания или в конструкторе было указано null в качестве
[ISchemeService](T_Tessa_Scheme_ISchemeService.htm), то это гарантирует, что
запросы к сервису будут значительно более эффективны. Инициализировать класс
любым другим способом имеет смысл только для использования в редакторе
карточек.
Методы
[UpdateAsync(CancellationToken)](M_Tessa_Cards_Metadata_CardCachedMetadata_UpdateAsync.htm)
и
[InvalidateAsync(CancellationToken)](M_Tessa_Cards_Metadata_CardCachedMetadata_InvalidateAsync.htm),
а также чтение всех свойств являются потокобезопасными.
##  __Конструкторы
