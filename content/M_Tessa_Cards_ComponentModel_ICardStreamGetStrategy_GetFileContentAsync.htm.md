...
C# __Копировать
    Task<(CardGetFileContentResponse response, Func<CancellationToken, ValueTask<Stream>> getContentFuncAsync)> GetFileContentAsync(
...
C++ __Копировать
    Task<ValueTuple<CardGetFileContentResponse^, Func<CancellationToken, ValueTask<Stream^>>^>>^ GetFileContentAsync(
    	CardGetFileContentRequest^ request, 
    	ICardMetadata^ cardMetadata, 
    	ISession^ session, 
