# CardExtensions.OrderManuallyForInsertion<T> \- метод
...
C# __Копировать
     public static T[] OrderManuallyForInsertion<T>(
    	this IEnumerable<T> rows
    )
    where T : ICardOrderable
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function OrderManuallyForInsertion(Of T As ICardOrderable) ( 
...
C++ __Копировать
    ...
    generic<typename T>
    where T : ICardOrderable
    static array<T>^ OrderManuallyForInsertion(
    	IEnumerable<T>^ rows
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member OrderManuallyForInsertion : 
