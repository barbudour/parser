# CommandLineProcessHost.OnChildStartingAsync - метод
...
C# __Копировать
     protected virtual Task OnChildStartingAsync(
    	string[] args,
    	CancellationToken cancellationToken = default
    )
VB __Копировать
     Protected Overridable Function OnChildStartingAsync ( 
    	args As String(),
    	Optional cancellationToken As CancellationToken = Nothing
    ) As Task
C++ __Копировать
     protected:
    virtual Task^ OnChildStartingAsync(
    	array<String^>^ args, 
    	CancellationToken cancellationToken = CancellationToken()
    )
F# __Копировать
     abstract OnChildStartingAsync : 
            args : string[] * 
