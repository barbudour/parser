# KrWorkflowActionMethods.CreateTaskInitMethodTemplate - метод
...
VB __Копировать
     Public Shared Function CreateTaskInitMethodTemplate ( 
    	storePath As String(),
    	Optional methodName As String = "InitTaskScript"
    ) As WorkflowActionMethodDescriptor
C++ __Копировать
     public:
    static WorkflowActionMethodDescriptor^ CreateTaskInitMethodTemplate(
    	array<String^>^ storePath, 
    	String^ methodName = L"InitTaskScript"
    )
F# __Копировать
     static member CreateTaskInitMethodTemplate : 
            storePath : string[] * 
            ?methodName : string 
    (* Defaults:
            let _methodName = defaultArg methodName "InitTaskScript"
    *)
...
(Optional)
    Имя метода. Значение по умолчанию: InitTaskScript.
