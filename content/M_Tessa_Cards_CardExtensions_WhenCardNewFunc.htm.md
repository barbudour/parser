# CardExtensions.WhenCardNewFunc - метод
...
C# __Копировать
     public static IExtensionPolicyContainer WhenCardNewFunc(
    	this IExtensionPolicyContainer policyContainer,
    	Func<ICardNewExtensionContext, bool> isAllowedFunc
    )
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function WhenCardNewFunc ( 
    	policyContainer As IExtensionPolicyContainer,
    	isAllowedFunc As Func(Of ICardNewExtensionContext, Boolean)
    ) As IExtensionPolicyContainer
C++ __Копировать
     public:
    [ExtensionAttribute]
    static IExtensionPolicyContainer^ WhenCardNewFunc(
    	IExtensionPolicyContainer^ policyContainer, 
    	Func<ICardNewExtensionContext^, bool>^ isAllowedFunc
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member WhenCardNewFunc : 
            policyContainer : IExtensionPolicyContainer * 
            isAllowedFunc : Func<ICardNewExtensionContext, bool> -> IExtensionPolicyContainer 
#### Параметры
policyContainer
...
isAllowedFunc
[Func](https://learn.microsoft.com/dotnet/api/system.func-2)<[ICardNewExtensionContext](T_Tessa_Cards_Extensions_ICardNewExtensionContext.htm),
