...
VB __Копировать
    <SerializableAttribute>
    Public NotInheritable Class CardFile
    	Inherits CardInfoStorageObject
C++ __Копировать
    [SerializableAttribute]
    public ref class CardFile sealed : public CardInfoStorageObject
F# __Копировать
     [<SealedAttribute>]
    [<SerializableAttribute>]
    type CardFile = 
...
##  __Конструкторы
[CardFile()](M_Tessa_Cards_CardFile__ctor.htm)|  Создаёт экземпляр класса и
пустое хранилище Dictionary<string, object>, декоратором для которого является
создаваемый объект.  
---|---  
[CardFile(Dictionary<String, Object>)](M_Tessa_Cards_CardFile__ctor_1.htm)|
Создаёт экземпляр класса с указанием хранилища, декоратором для которого
является создаваемый объект.  
[CardFile(IStorageObjectProvider)](M_Tessa_Cards_CardFile__ctor_2.htm)|
...
загружен.  
## __Методы
[AddNewVersionTag](M_Tessa_Cards_CardFile_AddNewVersionTag.htm)|  Добавляет
заданный тег в список тегов, соответствующих новой версии файла
[NewVersionTags](P_Tessa_Cards_CardFile_NewVersionTags.htm). Возвращает
признак того, что тег отсутствовал и был добавлен.  
---|---  
[Clean](M_Tessa_Cards_CardFile_Clean.htm)| Выполняет очистку хранилища от
избыточных данных.  
(Переопределяет
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[DeserializeOptions](M_Tessa_Cards_CardFile_DeserializeOptions.htm)|
Десериализует настройки [Options](P_Tessa_Cards_CardFile_Options.htm).
Возвращаемый объект не равен null. Если настройки не заданы, то возвращается
пустой объект.  
[EnsureCacheResolved](M_Tessa_Cards_CardFile_EnsureCacheResolved.htm)|
...
(Унаследован от
[Object](https://learn.microsoft.com/dotnet/api/system.object))  
[HasChanges](M_Tessa_Cards_CardFile_HasChanges.htm)|  Возвращает признак того,
что карточка файла содержит изменённые значения.  
[HasNewVersionTag](M_Tessa_Cards_CardFile_HasNewVersionTag.htm)|  Возвращает
признак того, что создаваемая версия будет содержать указанный тег в свойстве
[NewVersionTags](P_Tessa_Cards_CardFile_NewVersionTags.htm).  
[HasPendingStateChanges](M_Tessa_Cards_CardFile_HasPendingStateChanges.htm)|
...
[HasSize](M_Tessa_Cards_CardFile_HasSize.htm)|  Возвращает признак того, что
файл содержит информацию о длине контента файла, т.е. свойство
[Size](P_Tessa_Cards_CardFile_Size.htm) отлично от значения -1.  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[InvalidateLastVersion](M_Tessa_Cards_CardFile_InvalidateLastVersion.htm)|
Сбрасывает значение свойства
[LastVersion](P_Tessa_Cards_CardFile_LastVersion.htm), т.к. оно устарело в
связи с изменениями в версиях файла. Значение будет вновь установлено при
первом обращении к свойству.  
[IsEmpty](M_Tessa_Cards_CardFile_IsEmpty.htm)|  Возвращает признак того, что
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[RemoveChanges](M_Tessa_Cards_CardFile_RemoveChanges.htm)|  Выполняет удаление
информации по состояниям, из которой можно было бы определить, что файл
изменён. Возвращает признак того, что при этом были внесены изменения.  
[RemoveNewVersionTag](M_Tessa_Cards_CardFile_RemoveNewVersionTag.htm)|
...
(Унаследован от
[CardInfoStorageObject](T_Tessa_Cards_CardInfoStorageObject.htm))  
[RepairStorageAsync](M_Tessa_Cards_CardFile_RepairStorageAsync.htm)|
...
были изменения.  
[ResetSize](M_Tessa_Cards_CardFile_ResetSize.htm)|  Устанавливает признак
того, что файл не содержит информацию о длине контента файла, т.е. свойство
[Size](P_Tessa_Cards_CardFile_Size.htm) устанавливается как -1.  
[RevertReplacedState](M_Tessa_Cards_CardFile_RevertReplacedState.htm)|
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[SetCard](M_Tessa_Cards_CardFile_SetCard.htm)|  Связывает данные текущего
объекта с заданной карточкой файла. При это устанавливаются свойства
[RowID](P_Tessa_Cards_CardFile_RowID.htm),
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[SetOptions](M_Tessa_Cards_CardFile_SetOptions.htm)|  Устанавливает значение
свойства [Options](P_Tessa_Cards_CardFile_Options.htm) с выполнением
сериализации указанного хранилища.  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[TryGetCard](M_Tessa_Cards_CardFile_TryGetCard.htm)|  Возвращает карточку
файла или null, если карточка ещё не была задана.  
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[TryGetRequestInfo](M_Tessa_Cards_CardFile_TryGetRequestInfo.htm)|  Возвращает
дополнительную пользовательскую информацию по текущему объекту, передаваемую в
запросы
[CardGetFileContentRequest](T_Tessa_Cards_CardGetFileContentRequest.htm) и
[CardGetFileVersionsRequest](T_Tessa_Cards_CardGetFileVersionsRequest.htm), и
в запрос на загрузку списка подписей
...
[TryGetSectionRows](M_Tessa_Cards_CardFile_TryGetSectionRows.htm)|  Возвращает
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[TryGetVersions](M_Tessa_Cards_CardFile_TryGetVersions.htm)|  Возвращает
список версий файла или null, если список версий файла ещё не был задан или не
был загружен.  
[UpdateState](M_Tessa_Cards_CardFile_UpdateState.htm)|  Обновляет состояние
файла [State](P_Tessa_Cards_CardFile_State.htm) в зависимости от наличия
изменений во флагах или в данных карточки файла.  
...
(Унаследован от
[ValidationStorageObject](T_Tessa_Platform_Validation_ValidationStorageObject.htm))  
[ValidateInternal](M_Tessa_Cards_CardFile_ValidateInternal.htm)| Выполняет
...
##  __События
[StateChanged](E_Tessa_Cards_CardFile_StateChanged.htm)|  Событие, возникающее
при изменении состояния файла [State](P_Tessa_Cards_CardFile_State.htm).  
---|---  
## __Поля
