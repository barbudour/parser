# CardDeleteResponse.DeletedCardID - свойство
Идентификатор карточки удаления, которая содержит информацию об удалённой
карточке и может использоваться для её восстановления, или null, если карточка
была удалена без возможности восстановления или при удалении возникла ошибка.
## __Definition
 **Пространство имён:** [Tessa.Cards](N_Tessa_Cards.htm)  
 **Сборка:** Tessa (в Tessa.dll) Версия: 3.6.0.17
C# __Копировать
     public Guid? DeletedCardID { get; set; }
VB __Копировать
     Public Property DeletedCardID As Guid?
    	Get
    	Set
C++ __Копировать
     public:
    property Nullable<Guid> DeletedCardID {
    	Nullable<Guid> get ();
    	void set (Nullable<Guid> value);
    }
F# __Копировать
     member DeletedCardID : Nullable<Guid> with get, set
#### Значение свойства
[Nullable](https://learn.microsoft.com/dotnet/api/system.nullable-1)<[Guid](https://learn.microsoft.com/dotnet/api/system.guid)>
##  __См. также
#### Ссылки
[CardDeleteResponse - ](T_Tessa_Cards_CardDeleteResponse.htm)
[Tessa.Cards - пространство имён](N_Tessa_Cards.htm)
