# DefaultCardTypeExtensionSettings.IsOpenCardInDialog - поле
Нужно ли открывать карточку в режиме диалога.
## __Definition
 **Пространство имён:**
[Tessa.Extensions.Default.Shared.Cards](N_Tessa_Extensions_Default_Shared_Cards.htm)  
 **Сборка:** Tessa.Extensions.Default.Shared (в
Tessa.Extensions.Default.Shared.dll) Версия: 3.6.0.17
C# __Копировать
     public static readonly string IsOpenCardInDialog
VB __Копировать
     Public Shared ReadOnly IsOpenCardInDialog As String
C++ __Копировать
     public:
    static initonly String^ IsOpenCardInDialog
F# __Копировать
     static val IsOpenCardInDialog: string
#### Значение поля
[String](https://learn.microsoft.com/dotnet/api/system.string)
##  __См. также
#### Ссылки
[DefaultCardTypeExtensionSettings -
](T_Tessa_Extensions_Default_Shared_Cards_DefaultCardTypeExtensionSettings.htm)
[Tessa.Extensions.Default.Shared.Cards - пространство
имён](N_Tessa_Extensions_Default_Shared_Cards.htm)
