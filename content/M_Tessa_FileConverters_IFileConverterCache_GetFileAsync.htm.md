# IFileConverterCache.GetFileAsync - метод
...
C# __Копировать
    Task<(ValidationResult result, Func<CancellationToken, ValueTask<Stream>> getContentAsyncFunc, long size)> GetFileAsync(
...
C++ __Копировать
    Task<ValueTuple<ValidationResult^, Func<CancellationToken, ValueTask<Stream^>>^, long long>>^ GetFileAsync(
    	Guid convertedFileID, 
