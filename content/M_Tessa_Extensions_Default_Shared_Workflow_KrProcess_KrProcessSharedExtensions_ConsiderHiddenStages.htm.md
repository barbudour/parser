# KrProcessSharedExtensions.ConsiderHiddenStages - метод
...
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function ConsiderHiddenStages ( 
    	request As CardInfoStorageObject
    ) As Boolean
C++ __Копировать
     public:
    [ExtensionAttribute]
    static bool ConsiderHiddenStages(
    	CardInfoStorageObject^ request
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member ConsiderHiddenStages : 
...
Значение true, если в карточку должны быть загружены скрытые этапы маршрута,
иначе - false.
