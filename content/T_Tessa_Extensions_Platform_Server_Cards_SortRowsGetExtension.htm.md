...
C# __Копировать
     public sealed class SortRowsGetExtension : CardGetExtension
VB __Копировать
     Public NotInheritable Class SortRowsGetExtension
    	Inherits CardGetExtension
C++ __Копировать
     public ref class SortRowsGetExtension sealed : public CardGetExtension
F# __Копировать
     [<SealedAttribute>]
    type SortRowsGetExtension = 
        class
            inherit CardGetExtension
        end
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __[CardGetExtension](T_Tessa_Cards_Extensions_CardGetExtension.htm) __ SortRowsGetExtension
##  __Заметки
Расширение выполняется на этапе
[Finalize](T_Tessa_Extensions_ExtensionStage.htm), чтобы любые другие
расширения уже поменяли строки к этому моменту, а также чтобы они заполнили
виртуальные секции, если сортировка выполняется по такой секции.
## __Конструкторы
[SortRowsGetExtension](M_Tessa_Extensions_Platform_Server_Cards_SortRowsGetExtension__ctor.htm)|
Инициализирует новый экземпляр класса SortRowsGetExtension  
---|---  
##  __Методы
[AfterRequest](M_Tessa_Extensions_Platform_Server_Cards_SortRowsGetExtension_AfterRequest.htm)|  
(Переопределяет
