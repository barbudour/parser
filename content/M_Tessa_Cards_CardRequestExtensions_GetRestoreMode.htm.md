# CardRequestExtensions.GetRestoreMode - метод
...
C# __Копировать
     public static bool GetRestoreMode(
    	this CardDeleteRequest request
    )
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function GetRestoreMode ( 
    	request As CardDeleteRequest
    ) As Boolean
C++ __Копировать
     public:
    [ExtensionAttribute]
    static bool GetRestoreMode(
    	CardDeleteRequest^ request
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member GetRestoreMode : 
...
Признак того, что при удалении удалённой карточки одновременно выполняется
восстановление карточки, которая была удалена.
