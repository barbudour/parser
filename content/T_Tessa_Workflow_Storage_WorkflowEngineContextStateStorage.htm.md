# WorkflowEngineContextStateStorage - класс
...
C# __Копировать
    ...
    public sealed class WorkflowEngineContextStateStorage : StorageObject
VB __Копировать
    <SerializableAttribute>
    Public NotInheritable Class WorkflowEngineContextStateStorage
    	Inherits StorageObject
C++ __Копировать
    [SerializableAttribute]
    public ref class WorkflowEngineContextStateStorage sealed : public StorageObject
F# __Копировать
     [<SealedAttribute>]
    [<SerializableAttribute>]
    type WorkflowEngineContextStateStorage = 
        class
            inherit StorageObject
        end
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __[StorageObject](T_Tessa_Platform_Storage_StorageObject.htm) __ WorkflowEngineContextStateStorage
##  __Конструкторы
[WorkflowEngineContextStateStorage()](M_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage__ctor.htm)|
Инициализирует новый экземпляр класса WorkflowEngineContextStateStorage  
---|---  
[WorkflowEngineContextStateStorage(Dictionary<String,
Object>)](M_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage__ctor_1.htm)|
Создаёт экземпляр объекта с указанием хранилища данных.  
[WorkflowEngineContextStateStorage(IStorageObjectProvider)](M_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage__ctor_3.htm)|
Создаёт экземпляр объекта с указанием провайдера хранилища данных.  
[WorkflowEngineContextStateStorage(IEnumerable<WorkflowStorageBase>,
IWorkflowEngineSignal)](M_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage__ctor_2.htm)|
Создаёт экземпляр объекта по набору обработанных объектов процесса.  
## __Свойства
[ProcessItems](P_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage_ProcessItems.htm)|
Список объектов, содержащих информацию об обработанных объектах процесса.  
---|---  
[ProcessLocked](P_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage_ProcessLocked.htm)|
Определяет, был ли заблокирован процесс при запуске обработки сигнала.  
[SignalHash](P_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage_SignalHash.htm)|
Хеш сигнала, при обработке которого возникла ошибка, или null, если ошибка
возникла вне обработки сигнала.  
[SignalType](P_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage_SignalType.htm)|
Тип сигнала, при обработке которого возникла ошибка, или null, если ошибка
возникла вне обработки сигнала.  
[StartSignalHash](P_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage_StartSignalHash.htm)|
Хеш сигнала, который был использован при запуске обработки процесса, или null,
если тип сигнала запуска не задан.  
[StartSignalType](P_Tessa_Workflow_Storage_WorkflowEngineContextStateStorage_StartSignalType.htm)|
Тип сигнала, который был использован при запуске обработки процесса, или null,
если тип сигнала запуска не задан.  
## __Методы
