...
C# __Копировать
    [RouteAttribute("api/v1/workplaces")]
    ...
    public sealed class WorkplacesController : Controller
VB __Копировать
    <RouteAttribute("api/v1/workplaces")>
    <AllowAnonymousAttribute>
    <ApiControllerAttribute>
    ...
    <ProducesResponseTypeAttribute(401)>
    <ProducesResponseTypeAttribute(403)>
    Public NotInheritable Class WorkplacesController
    	Inherits Controller
C++ __Копировать
    [RouteAttribute(L"api/v1/workplaces")]
    [AllowAnonymousAttribute]
    [ApiControllerAttribute]
    [ProducesErrorResponseTypeAttribute(typeof(PlainValidationResult))]
    [ProducesResponseTypeAttribute(400)]
    [ProducesResponseTypeAttribute(401)]
    [ProducesResponseTypeAttribute(403)]
    public ref class WorkplacesController sealed : public Controller
F# __Копировать
     [<SealedAttribute>]
    [<RouteAttribute("api/v1/workplaces")>]
    [<AllowAnonymousAttribute>]
    ...
    [<ProducesResponseTypeAttribute(401)>]
    [<ProducesResponseTypeAttribute(403)>]
    type WorkplacesController = 
        class
            inherit Controller
        end
Inheritance
    [Object](https://learn.microsoft.com/dotnet/api/system.object) __[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase) __[Controller](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controller) __ WorkplacesController
##  __Конструкторы
[WorkplacesController](M_Tessa_Web_Controllers_WorkplacesController__ctor.htm)|
Инициализирует новый экземпляр класса WorkplacesController  
---|---  
##  __Свойства
...
(Унаследован от
[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase))  
[Delete](M_Tessa_Web_Controllers_WorkplacesController_Delete.htm)|  Удаляет
одно или несколько рабочих мест. Метод доступен только администраторам.  
[DeleteUserMetadata](M_Tessa_Web_Controllers_WorkplacesController_DeleteUserMetadata.htm)|
Удаляет метаинформацию рабочих мест для текущего пользователя, такую как
информацию по добавленным поисковым запросам.  
...
(Унаследован от
[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase))  
[Get](M_Tessa_Web_Controllers_WorkplacesController_Get.htm)|  Возвращает
метаинформацию по заданному рабочему месту, необходимую для отображения
рабочего места. Рабочее место должно быть доступно текущему пользователю.  
[GetAll](M_Tessa_Web_Controllers_WorkplacesController_GetAll.htm)|  Возвращает
метаинформацию по всем рабочим местам, доступным текущему пользователю.  
[GetUserMetadata](M_Tessa_Web_Controllers_WorkplacesController_GetUserMetadata.htm)|
Возвращает метаинформацию рабочих мест для текущего пользователя, такую как
информацию по добавленным поисковым запросам. Для десериализации используйте
клиентское API desktop-клиента.  
...
(Унаследован от
[ControllerBase](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.controllerbase))  
[PostGetModels](M_Tessa_Web_Controllers_WorkplacesController_PostGetModels.htm)|
Возвращает метаинформацию по одному или нескольким рабочим местам. Такая
метаинформация может использоваться для изменения рабочих мест (при наличии
административных прав).  
[PostImport](M_Tessa_Web_Controllers_WorkplacesController_PostImport.htm)|
Импортирует заданные рабочие места. Метод доступен только администраторам.  
[PostSaveProperties](M_Tessa_Web_Controllers_WorkplacesController_PostSaveProperties.htm)|
Сохраняет настройки представлений в рабочих местах для текущего пользователя.
Это включает настройки по сортировке, группировке и скрытию столбцов
представлений, а также относительные размеры областей с представлениями в
master-detail отчётах.  
[PostSaveSettings](M_Tessa_Web_Controllers_WorkplacesController_PostSaveSettings.htm)|
Сохраняет настройки одного или нескольких рабочих мест для текущего
пользователя. Например, это ширина дерева рабочего места, режим отображения
пустых папок и др. Изменения недоступны других пользователям.  
[PostStore](M_Tessa_Web_Controllers_WorkplacesController_PostStore.htm)|
Сохраняет одно или несколько рабочих мест. Метод доступен только
администраторам.  
[PostStoreUserMetadata](M_Tessa_Web_Controllers_WorkplacesController_PostStoreUserMetadata.htm)|
Сохраняет метаинформацию рабочего места для текущего пользователя. Например,
добавляет поисковый запрос или папку в дерево рабочего места. Изменения
недоступны других пользователям.  
