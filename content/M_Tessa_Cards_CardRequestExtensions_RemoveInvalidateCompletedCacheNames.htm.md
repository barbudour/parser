# CardRequestExtensions.RemoveInvalidateCompletedCacheNames - метод
...
C# __Копировать
     public static void RemoveInvalidateCompletedCacheNames(
    	this ICardRequestExtensionContext context,
    	params string[] cacheNames
    )
VB __Копировать
    <ExtensionAttribute>
    Public Shared Sub RemoveInvalidateCompletedCacheNames ( 
...
C++ __Копировать
     public:
    [ExtensionAttribute]
    static void RemoveInvalidateCompletedCacheNames(
    	ICardRequestExtensionContext^ context, 
    	... array<String^>^ cacheNames
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member RemoveInvalidateCompletedCacheNames : 
...
cacheNames [String](https://learn.microsoft.com/dotnet/api/system.string)[]
     Имена кэшей, которые не были сброшены. Значение null и пустой список идентичны. 
