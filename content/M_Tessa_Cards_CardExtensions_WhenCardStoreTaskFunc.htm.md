# CardExtensions.WhenCardStoreTaskFunc - метод
...
C# __Копировать
     public static IExtensionPolicyContainer WhenCardStoreTaskFunc(
    	this IExtensionPolicyContainer policyContainer,
    	Func<ICardStoreTaskExtensionContext, bool> isAllowedFunc
    )
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function WhenCardStoreTaskFunc ( 
    	policyContainer As IExtensionPolicyContainer,
    	isAllowedFunc As Func(Of ICardStoreTaskExtensionContext, Boolean)
    ) As IExtensionPolicyContainer
C++ __Копировать
     public:
    [ExtensionAttribute]
    static IExtensionPolicyContainer^ WhenCardStoreTaskFunc(
    	IExtensionPolicyContainer^ policyContainer, 
    	Func<ICardStoreTaskExtensionContext^, bool>^ isAllowedFunc
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member WhenCardStoreTaskFunc : 
            policyContainer : IExtensionPolicyContainer * 
            isAllowedFunc : Func<ICardStoreTaskExtensionContext, bool> -> IExtensionPolicyContainer 
#### Параметры
policyContainer
...
isAllowedFunc
[Func](https://learn.microsoft.com/dotnet/api/system.func-2)<[ICardStoreTaskExtensionContext](T_Tessa_Cards_Extensions_ICardStoreTaskExtensionContext.htm),
