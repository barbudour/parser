# CardTaskCompletionOptionSettings.PreparedNewCard - свойство
Возвращает или задаёт массив байт представляющий сериализованную карточку-
заготовку.
## __Definition
 **Пространство имён:** [Tessa.Cards](N_Tessa_Cards.htm)  
 **Сборка:** Tessa (в Tessa.dll) Версия: 3.6.0.17
C# __Копировать
     public byte[]? PreparedNewCard { get; set; }
VB __Копировать
     Public Property PreparedNewCard As Byte()
    	Get
    	Set
C++ __Копировать
     public:
    property array<unsigned char>^ PreparedNewCard {
    	array<unsigned char>^ get ();
    	void set (array<unsigned char>^ value);
    }
F# __Копировать
     member PreparedNewCard : byte[] with get, set
#### Значение свойства
[Byte](https://learn.microsoft.com/dotnet/api/system.byte)[]
##  __См. также
#### Ссылки
[CardTaskCompletionOptionSettings -
](T_Tessa_Cards_CardTaskCompletionOptionSettings.htm)
[Tessa.Cards - пространство имён](N_Tessa_Cards.htm)
[PreparedNewCardSignature](P_Tessa_Cards_CardTaskCompletionOptionSettings_PreparedNewCardSignature.htm)
