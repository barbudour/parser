# CardControlSettings.HorizontalInterval - поле
Значение по умолчанию для горизонтального интервала между элементами
управления, которые содержатся в блоке.
## __Definition
 **Пространство имён:** [Tessa.Cards](N_Tessa_Cards.htm)  
 **Сборка:** Tessa (в Tessa.dll) Версия: 3.6.0.17
C# __Копировать
     public const int HorizontalInterval = 5
VB __Копировать
     Public Const HorizontalInterval As Integer = 5
C++ __Копировать
     public:
    literal int HorizontalInterval = 5
F# __Копировать
     static val mutable HorizontalInterval: int
#### Значение поля
[Int32](https://learn.microsoft.com/dotnet/api/system.int32)
##  __См. также
#### Ссылки
[CardControlSettings - ](T_Tessa_Cards_CardControlSettings.htm)
[Tessa.Cards - пространство имён](N_Tessa_Cards.htm)
