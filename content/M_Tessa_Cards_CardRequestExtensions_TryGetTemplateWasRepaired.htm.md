# CardRequestExtensions.TryGetTemplateWasRepaired - метод
...
C# __Копировать
     public static bool TryGetTemplateWasRepaired(
    	this Card card
    )
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function TryGetTemplateWasRepaired ( 
    	card As Card
    ) As Boolean
C++ __Копировать
     public:
    [ExtensionAttribute]
    static bool TryGetTemplateWasRepaired(
    	Card^ card
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member TryGetTemplateWasRepaired : 
            card : Card -> bool 
#### Параметры
...
true, если карточка шаблона была исправлена после изменения схемы данных;
false, если исправление структуры карточки шаблона не потребовалось.
