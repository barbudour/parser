...
C# __Копировать
    ...
    public sealed class WorkflowQueue : CardInfoStorageObject, 
    	ICloneable
VB __Копировать
    <SerializableAttribute>
    Public NotInheritable Class WorkflowQueue
    	Inherits CardInfoStorageObject
    	Implements ICloneable
C++ __Копировать
    [SerializableAttribute]
    public ref class WorkflowQueue sealed : public CardInfoStorageObject, 
    	ICloneable
F# __Копировать
     [<SealedAttribute>]
    [<SerializableAttribute>]
...
(Унаследован от
[CardInfoStorageObject](T_Tessa_Cards_CardInfoStorageObject.htm))  
[IsEmpty](P_Tessa_Cards_Workflow_WorkflowQueue_IsEmpty.htm)|  Возвращает
признак того, что объект не содержит сигналов.  
[Items](P_Tessa_Cards_Workflow_WorkflowQueue_Items.htm)|  Список сигналов в
порядке их поступления.  
## __Методы
[Add](M_Tessa_Cards_Workflow_WorkflowQueue_Add.htm)|  Добавляет запись в
очередь, которая описывает сигнал.  
---|---  
[AddSignal](M_Tessa_Cards_Workflow_WorkflowQueue_AddSignal.htm)|  Добавляет
запись в очередь, которая описывает сигнал.  
[Clean](M_Tessa_Cards_Workflow_WorkflowQueue_Clean.htm)| Выполняет очистку
...
[Clear](M_Tessa_Cards_Workflow_WorkflowQueue_Clear.htm)|  Очищает очередь
сигналов.  
[ClearCache](M_Tessa_Platform_Storage_StorageObject_ClearCache.htm)|  Очищает
внутренний кэш декораторов.  
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[Clone](M_Tessa_Cards_Workflow_WorkflowQueue_Clone.htm)|  Выполняет глубокое
...
(Унаследован от [StorageObject](T_Tessa_Platform_Storage_StorageObject.htm))  
[EnsureCacheResolved](M_Tessa_Cards_Workflow_WorkflowQueue_EnsureCacheResolved.htm)|
...
(Унаследован от
[Object](https://learn.microsoft.com/dotnet/api/system.object))  
[HasItemsToHandleAsync(Func<WorkflowQueueItem, CancellationToken,
ValueTask<Boolean>>, CancellationToken,
WorkflowQueueEventType[])](M_Tessa_Cards_Workflow_WorkflowQueue_HasItemsToHandleAsync.htm)|
Возвращает признак того, что в очереди присутствует хотя бы один элемент
очереди, который обрабатывается для хотя бы одного из заданных событий.
Учитываются только ещё необработанные элементы.  
[HasItemsToHandleAsync(Func<WorkflowQueueItem, CancellationToken,
ValueTask<Boolean>>, WorkflowQueueEventType,
CancellationToken)](M_Tessa_Cards_Workflow_WorkflowQueue_HasItemsToHandleAsync_1.htm)|
Возвращает признак того, что в очереди присутствует хотя бы один элемент
очереди, который обрабатывается для заданного события. Учитываются только ещё
...
(Унаследован от
[CardInfoStorageObject](T_Tessa_Cards_CardInfoStorageObject.htm))  
[TryGetItems](M_Tessa_Cards_Workflow_WorkflowQueue_TryGetItems.htm)|
Возвращает список сигналов в порядке их поступления или null, если список ещё
не был задан.  
...
(Унаследован от
[ValidationStorageObject](T_Tessa_Platform_Validation_ValidationStorageObject.htm))  
[ValidateInternal](M_Tessa_Cards_Workflow_WorkflowQueue_ValidateInternal.htm)|
Выполняет валидацию текущего объекта и всех его дочерних объектов.  
(Переопределяет
