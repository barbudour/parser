# KrProcessSharedExtensions.TryGetGlobalTiles - метод
...
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function TryGetGlobalTiles ( 
    	response As InitializationResponse,
    	<OutAttribute> ByRef globalTiles As List(Of KrTileInfo)
    ) As Boolean
C++ __Копировать
     public:
    [ExtensionAttribute]
    static bool TryGetGlobalTiles(
    	InitializationResponse^ response, 
    	[OutAttribute] List<KrTileInfo^>^% globalTiles
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member TryGetGlobalTiles : 
            response : InitializationResponse * 
            globalTiles : List<KrTileInfo> byref -> bool 
#### Параметры
response
...
[List](https://learn.microsoft.com/dotnet/api/system.collections.generic.list-1)<[KrTileInfo](T_Tessa_Extensions_Default_Shared_Workflow_KrProcess_KrTileInfo.htm)>
    Возвращаемое значение. Коллекция объектов содержащих информацию о глобальных тайлах маршрутов.
#### Возвращаемое значение
[Boolean](https://learn.microsoft.com/dotnet/api/system.boolean)  
Значение true, если объект содержал коллекцию объектов содержащих информацию о
глобальных тайлах маршрутов, иначе - false.
