# KrProcessSharedExtensions.GetKrProcessLaunchResult - метод
...
VB __Копировать
    <ExtensionAttribute>
    Public Shared Function GetKrProcessLaunchResult ( 
    	storage As CardInfoStorageObject
    ) As KrProcessLaunchResult
C++ __Копировать
     public:
    [ExtensionAttribute]
    static KrProcessLaunchResult^ GetKrProcessLaunchResult(
    	CardInfoStorageObject^ storage
    )
F# __Копировать
     [<ExtensionAttribute>]
    static member GetKrProcessLaunchResult : 
            storage : CardInfoStorageObject -> KrProcessLaunchResult 
#### Параметры
storage [CardInfoStorageObject](T_Tessa_Cards_CardInfoStorageObject.htm)
    Хранилище содержащее результаты запуска процесса.
#### Возвращаемое значение
[KrProcessLaunchResult](T_Tessa_Extensions_Default_Shared_Workflow_KrProcess_KrProcessLaunchResult.htm)  
Объект содержащий результат запуска процесса или значение по умолчанию для
типа, если указанный объект его не содержит.
