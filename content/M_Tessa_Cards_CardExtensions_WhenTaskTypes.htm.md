# CardExtensions.WhenTaskTypes(IExtensionPolicyContainer, Guid[]) - метод
Регистрирует политику фильтрации выполнения методов расширений по
идентификатору типа задания, который входит в заданный список идентификаторов.
Если тип задания неизвестен, то метод расширения не выполняется. Для того,
...
taskTypeIDs [Guid](https://learn.microsoft.com/dotnet/api/system.guid)[]
     Список идентификаторов типов заданий, в который должен входить идентификатор типа задания, для которого выполняется метод расширения. 
