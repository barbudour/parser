# CardExtensions.WhenFileTypes(IExtensionPolicyContainer, Guid[]) - метод
Регистрирует политику фильтрации выполнения методов расширений по
идентификатору типа файла, который входит в заданный список идентификаторов.
Если тип файла неизвестен, то метод расширения не выполняется. Для того, чтобы
...
fileTypeIDs [Guid](https://learn.microsoft.com/dotnet/api/system.guid)[]
     Список идентификаторов типов файлов, в который должен входить идентификатор типа файла, для которого выполняется метод расширения. 
