# WfHelper.DisableChildResolutionDateCheckField - поле
Название поля для указания признака того, что в резолюциях отключается
проверка на соответствие даты запланированного завершения дочерней резолюции к
дате запланированного завершения родительской резолюции в секциях
[CardTypeSettingsSection](F_Tessa_Extensions_Default_Shared_Workflow_Wf_WfHelper_CardTypeSettingsSection.htm)
и
[DocTypeSettingsSection](F_Tessa_Extensions_Default_Shared_Workflow_Wf_WfHelper_DocTypeSettingsSection.htm).
## __Definition
 **Пространство имён:**
[Tessa.Extensions.Default.Shared.Workflow.Wf](N_Tessa_Extensions_Default_Shared_Workflow_Wf.htm)  
 **Сборка:** Tessa.Extensions.Default.Shared (в
Tessa.Extensions.Default.Shared.dll) Версия: 3.6.0.17
C# __Копировать
     public const string DisableChildResolutionDateCheckField = "DisableChildResolutionDateCheck"
VB __Копировать
     Public Const DisableChildResolutionDateCheckField As String = "DisableChildResolutionDateCheck"
C++ __Копировать
     public:
    literal String^ DisableChildResolutionDateCheckField = "DisableChildResolutionDateCheck"
F# __Копировать
     static val mutable DisableChildResolutionDateCheckField: string
#### Значение поля
[String](https://learn.microsoft.com/dotnet/api/system.string)
##  __См. также
#### Ссылки
[WfHelper - ](T_Tessa_Extensions_Default_Shared_Workflow_Wf_WfHelper.htm)
[Tessa.Extensions.Default.Shared.Workflow.Wf - пространство
имён](N_Tessa_Extensions_Default_Shared_Workflow_Wf.htm)
